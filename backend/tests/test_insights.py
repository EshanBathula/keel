...
def test_category_qoq_trend_computed_numbers(client, auth):
    ...
    for offset in (-6, -5, -4):
        client.post(
            "/api/transactions",
            headers=auth,
            json={"type": "expense", "amount": 1000, "category": "Marketing", "date": month_str(offset)},
        )
        client.post(
            "/api/transactions",
            headers=auth,
            json={"type": "expense", "amount": 2000, "category": "Rent", "date": month_str(offset)},
        )
    for offset in (-3, -2, -1):
        client.post(
            "/api/transactions",
            headers=auth,
            json={"type": "expense", "amount": 1500, "category": "Marketing", "date": month_str(offset)},
        )
        client.post(
            "/api/transactions",
            headers=auth,
...
def test_small_category_growth_not_flagged(client, auth):
    ...
    for offset in (-6, -5, -4):
        client.post(
            "/api/transactions",
            headers=auth,
            json={"type": "expense", "amount": 33, "category": "Snacks", "date": month_str(offset)},
        )
    for offset in (-3, -2, -1):
        client.post(
            "/api/transactions",
            headers=auth,
            json={"type": "expense", "amount": 66, "category": "Snacks", "date": month_str(offset)},
        )
...
def test_forecast_cash_low_emits_critical_insight(client, auth):
    ...
    for w in range(1, 17):
        ...
        client.post(
            "/api/transactions", headers=auth, json={"type": "income", "amount": 100, "category": "Services", "date": d}
        )
        client.post(
            "/api/transactions", headers=auth, json={"type": "expense", "amount": 1500, "category": "Ops", "date": d}
        )
...
def test_healthy_business_no_cash_low_insight(client, auth):
    ...
    for w in range(1, 17):
        ...
        client.post(
            "/api/transactions",
            headers=auth,
            json={"type": "income", "amount": 2000, "category": "Services", "date": d},
        )
        client.post(
            "/api/transactions", headers=auth, json={"type": "expense", "amount": 500, "category": "Ops", "date": d}
        )
