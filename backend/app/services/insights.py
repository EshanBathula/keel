...
from .analytics import category_breakdown, compute_kpis, month_key, monthly_series, shift_month, top_customers
    ...
    for cid, days in days_by_customer.items():
        ...
        if len(days) >= LATE_PAYER_MIN_INVOICES and avg >= LATE_PAYER_MIN_AVG_DAYS:
            ...
            outstanding_cents = sum(
                i.amount_cents for i in customer.invoices if i.status in (InvoiceStatus.sent, InvoiceStatus.overdue)
            )
            flagged.append(
                {
                    "name": customer.name,
                    "avg_days_late": round(avg),
                    "n_invoices": len(days),
...
def _category_qoq_growth(db: Session, user_id: int, today: date) -> list[dict]:
    ...
    txs = db.scalars(
        select(Transaction).where(Transaction.user_id == user_id, Transaction.type == TxType.expense)
    ).all()
    for tx in txs:
...
def generate_insights(db: Session, user_id: int, today: date | None = None) -> list[dict]:
    ...
    exp_now = {
        c["category"]: c["total"] for c in category_breakdown(db, user_id, TxType.expense, months=1, today=today)
    }
    exp_hist = {
        c["category"]: c["total"] for c in category_breakdown(db, user_id, TxType.expense, months=6, today=today)
    }
