...
from ..schemas import KPIs, MonthlyPoint, ForecastResponse, Insight, ScenarioRequest
from ..services import analytics as svc
from ..services.forecast import forecast as run_forecast, scenario as run_scenario
...
def scenario(payload: ScenarioRequest, horizon: int = Query(6, ge=1, le=12),
             db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    return run_scenario(
        db, user.id,
        ...
        horizon_months=horizon, today=user_today(user),
    )
