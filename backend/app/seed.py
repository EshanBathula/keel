...
def run():
    ...
    try:
        ...
        if existing:
            ...
            return
        user = User(email="demo@keel.app", password_hash=hash_password("demopassword"), business_name="Keel Demo Co.")
        db.add(user)
        db.flush()

        customers = [Customer(user_id=user.id, name=n, email=f"billing@{n.split()[0].lower()}.com") for n in CUSTOMERS]
        ...
        for m_back in range(11, -1, -1):
            ...
            for cust, w in zip(customers, weights, strict=True):
            ...
            for cat, base in EXPENSE_CATS.items():
                ...
                db.add(
                    Transaction(
                        ...
                        amount=amt,
                        category=cat,
                        description=f"{cat} — monthly",
                        date=anchor.replace(day=min(5, 28)),
                    )
                )
